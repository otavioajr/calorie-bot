...
export async function runConsensusPromotion(
  ...
  for (const cluster of clustersByKey.values()) {
    ...
    const { error: insertError } = await supabase.from('products').insert({
      ...
      serving_size_g: sample.serving_size_g ?? null,
      serving_display: sample.serving_display ?? null,
      ...
      fiber_per_100g: sample.fiber_per_100g ?? null,
      sodium_per_100g: sample.sodium_per_100g ?? null,
